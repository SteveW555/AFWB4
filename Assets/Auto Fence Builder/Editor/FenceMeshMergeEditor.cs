...
public class FenceMeshMergeEditor : Editor
{
    ...
    private void SaveMergedMeshes(List<GameObject> finishedGameObjects)
    {
        string dateStr = GetPartialTimeString(true);
        string path, folderName = "Meshes-Merged  " + dateStr;

        if (!Directory.Exists("Assets/Auto Fence Builder/FinishedFenceAssets"))
        {
            AssetDatabase.CreateFolder("Assets/Auto Fence Builder", "FinishedFenceAssets");
        }
        AssetDatabase.CreateFolder("Assets/Auto Fence Builder/FinishedFenceAssets", folderName);
        path = "Assets/Auto Fence Builder/FinishedFenceAssets/" + folderName + "/";
